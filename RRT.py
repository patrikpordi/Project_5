...
def rrt(start,goal,goal_tolerance, max_iterations,step_size):#step_size, ,  goal_tolerance goal,
    ...
    for i in range(1, max_iterations):
        ...
        # k=collides(q_near,q_new,obstacles)
        
        rode=tuple(bresenham_line(q_near[0],q_near[1],q_new[0],q_new[1]))
       

        if((q_new not in pixels) and q_new not in obstacles and not any(arr[x, y] == 1 for x, y in rode)):#and (q_new not in obstacles.keys() )
...
start=(10,10)
goal=(300,15)
pi,c,pa,path=rrt(start,goal,10,5000,10)
